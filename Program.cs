...
Player player = new Player(3.456f, 2.345f, 1.523f, MathF.PI / 3.0f);
...
void render()
{
    ...
    //Parallel.For(0, RENDER_WIDTH / 2, (i) =>
    {
        ...
        {
            float x = player.X + t * MathF.Cos(angle);
            float y = player.Y + t * MathF.Sin(angle);

            //if (map[float_to_int(cx) + float_to_int(cy) * map_w] != ' ') break;

            int pix_x = Convert.ToInt32(x * rect_w);
            int pix_y = Convert.ToInt32(y * rect_h);
            ...
            if (map.IsEmpty(Util.float_to_int(x), Util.float_to_int(y))) continue;
            
            int cell = map.Get(Util.float_to_int(x), Util.float_to_int(y));
            int texid = cell;

            int column_height = Util.float_to_int(RENDER_HEIGHT / (t * MathF.Cos(angle - player.A)));

            int x_texcoord = wall_x_texcoord(x, y);
...
int wall_x_texcoord(float x, float y)
{
    float hitx = x - MathF.Floor(x + 0.5f); // hitx and hity contain (signed) fractional parts of cx and cy,
    float hity = y - MathF.Floor(y + 0.5f); // they vary between -0.5 and +0.5, and one of them is supposed to be very close to 0
